...
sub getStats {
  my ($algorithm, $numberDays) = @_;
  my $statistics = {};

  foreach my $fileName (`ls *${algorithm}*_days_${numberDays}_*.allHV`) {
    chomp($fileName);
...
sub getRankings {
  my ($algorithm, $numberRuns, $numberDays) = @_;
  my @fileNames = `ls *${algorithm}*_days_${numberDays}_*.allHV`;
  my $rankings = {};
...
my $statistics = getStats $algorithm, $numberDays;
my $rankings = getRankings $algorithm, $numberRuns, $numberDays;
